...
static int32_t run_xrt_cmd(xrt_cmd_data_t *cmd)
{
  ...
  if (!cmd || cmd->cu_status == SK_STATUS_BAD)
   ...
   XmaCUCmdObj cu_cmd = xma_plg_schedule_work_item(*(cmd->xma_session), (char*)(cmd->registers), DEC_REGMAP_SIZE*sizeof(uint32_t), &ret);

    while (ret < 0) {
      //sleep(1);
      cu_cmd = xma_plg_schedule_work_item(*(cmd->xma_session), (char*)(cmd->registers), DEC_REGMAP_SIZE*sizeof(uint32_t), &ret);
    };

  ret = xma_plg_is_work_item_done(*(cmd->xma_session), 5000);
  if (ret != XMA_SUCCESS) {
    xma_logmsg(XMA_ERROR_LOG, XMA_VCU_DECODER, "*****ERROR:: Decoder stopped responding (%s)*****\n", __func__);
...
#ifndef ZERO_COPY
static int allocate_buff_pool(XmaSession xma_session, obuff_pool_ctx_t *buf_ctx)
{
  int ret, j;
  uint32_t i;
  uint64_t paddr;
  ...
  {
    buf_ctx->buf_objs[i] = xma_plg_buffer_alloc(g_handle, buf_ctx->buf_size, false, &ret);
    if (ret != XMA_SUCCESS) {
      ret = XMA_ERROR;
      break;
    }
...
fill_free_outbuffer_indexes (xrt_cmd_data_t *cmd,  sk_payload_data *data)
{
  ...
  if (free_pool_size > MAX_OUT_INFOS) {
    xma_logmsg(XMA_WARNING_LOG, XMA_VCU_DECODER, "free pool size is greater than max out infos array...restricting to max out infos\n");
    free_pool_size = MAX_OUT_INFOS;
  }
  data->valid_oidxs = free_pool_size;

  for (i = 0; i < free_pool_size; i++) {
    cmd->o_xvbm_buffer_handle = xvbm_buffer_pool_entry_alloc(cmd->obuf_ctx->out_pool_handle);
     if (cmd->o_xvbm_buffer_handle) {
       cmd->obuf_ctx->buf_count = xvbm_buffer_pool_num_buffers_get(cmd->o_xvbm_buffer_handle);
       paddr = xvbm_buffer_get_paddr(cmd->o_xvbm_buffer_handle);
       if (!paddr) {
         xma_logmsg(XMA_ERROR_LOG, XMA_VCU_DECODER, "%s : Error - no free output buffer available\n", __func__);
         return;
       }

       data->obuf_info[i].freed_obuf_index = xvbm_buffer_get_id(cmd->o_xvbm_buffer_handle);
       data->obuf_info[i].freed_obuf_paddr = paddr;
       data->obuf_info[i].freed_obuf_size = xvbm_buffer_get_size(cmd->o_xvbm_buffer_handle);
     } else {
       data->obuf_info[i].freed_obuf_index = 0xBAD;
       data->obuf_info[i].freed_obuf_paddr = 0;
       data->obuf_info[i].freed_obuf_size = 0;
       xma_logmsg(XMA_ERROR_LOG, XMA_VCU_DECODER, "Error: (%s) obuf_ctx Buffer Pool full - no free buffer available\n", __func__);
     }
  }
}
...
static bool validate_params_data(dec_params_t *data)
{
  ...
  if (data->codec_type > 1) {
    xma_logmsg(XMA_ERROR_LOG, XMA_VCU_DECODER, "Invalid codec type specified. Valid: 0 or 1\n");
    return false;
  }

  if (data->low_latency > 1) {
    xma_logmsg(XMA_ERROR_LOG, XMA_VCU_DECODER, "Invalid low latency flag. Valid: 0 or 1\n");
    return false;
  }

  if (data->splitbuff_mode > 1) {
    xma_logmsg(XMA_ERROR_LOG, XMA_VCU_DECODER, "Invalid split buffer mode flag. Valid: 0 or 1\n");
    return false;
  }

  if (data->entropy_buffers_count < 2 || data->entropy_buffers_count > 10) {
     xma_logmsg(XMA_WARNING_LOG, XMA_VCU_DECODER, "invalid entropy_buffers_count. Valid range: [2..10], setting default to 2\n");
  ...
  if (data->codec_type == 0) {
    /* H264 */
    if ((data->profile != AVC_PROFILE_IDC_BASELINE) && (data->profile != AVC_PROFILE_IDC_MAIN) &&
    ...
    switch (data->level) {
      ...
      case 20:case 21:case 22:case 30:case 31:case 32:
      case 40:case 41:case 42:case 50:case 51:case 52:
...
static int prepare_sk_cmd(xrt_cmd_data_t *cmd, enum cmd_type type, int64_t pts)
{
  ...
  switch (type)
  {
    case VCU_PREINIT:
      ...
      cmd->paramsbuf_ctx->buff_obj = xma_plg_buffer_alloc(*(cmd->xma_session), cmd->paramsbuf_ctx->buf_size, false, &ret);
      if (ret != XMA_SUCCESS) {
        ret = XMA_ERROR;
        break;
      }
      ...
      cmd->registers[++idx] = ((uint64_t)(cmd->paramsbuf_ctx->buff_obj.paddr) >> 32) & 0xFFFFFFFF;
      ...
      if ((param = getParameter (props->params, props->param_cnt, "entropy_buffers_count")))
      ...
      if ((param = getParameter (props->params, props->param_cnt, "splitbuff_mode")))
      ...
      memcpy ( cmd->paramsbuf_ctx->buff_obj.data, &(cmd->params_data), cmd->paramsbuf_ctx->buf_size);
      ret = xma_plg_buffer_write (*(cmd->xma_session), cmd->paramsbuf_ctx->buff_obj, cmd->paramsbuf_ctx->buf_size, 0);
    ...
    case VCU_INIT:
      ...
      for (int i = 0; i < MAX_IBUFFS; i++) {
        cmd->ibuf_ctx->buff_obj_arr[i] = xma_plg_buffer_alloc(*(cmd->xma_session), cmd->ibuf_ctx->buf_size, false, &ret);
        if (ret != XMA_SUCCESS) {
          ret = XMA_ERROR;
          goto err1;
        }
        ...
        cmd->registers[++idx] = ((uint64_t)(cmd->ibuf_ctx->buff_obj_arr[i].paddr) >> 32) & 0xFFFFFFFF;
...
//      cmd->obuf_ctx->out_pool_handle = xvbm_buffer_pool_create_by_device_id(cmd->xma_session->hw_session.dev_index, data.obuff_num, data.obuff_size, 0);
      cmd->obuf_ctx->out_pool_handle = xvbm_buffer_pool_create(xma_plg_get_dev_handle(*(cmd->xma_session)), data.obuff_num, data.obuff_size, 0);
      ...
      cmd->obuf_ctx->parent_buff_obj = xma_plg_buffer_alloc(*(cmd->xma_session), cmd->obuf_ctx->buf_count * sizeof (uint64_t), false, &ret);
      if(ret != XMA_SUCCESS) {
        xma_logmsg(XMA_ERROR_LOG, XMA_VCU_DECODER, "parent_buff_handle failed for outbuf\n");
        ret = XMA_ERROR;
        goto err3;
      }
      ...
      if (!cmd->obuf_ctx->parent_buff_paddr) {
        xma_logmsg(XMA_ERROR_LOG, XMA_VCU_DECODER, "parent_buff_paddr for outbuf failed\n");
      ...
      for (i = 0; i < data.obuff_num; i++) {
        ...
        if (o_handle) {
          ...
          ret = xma_plg_buffer_write (*(cmd->xma_session), cmd->obuf_ctx->parent_buff_obj, sizeof (uint64_t), offset);
          if (ret != XMA_SUCCESS) {
            ret = XMA_ERROR;
            goto err4;
          }
...
err1:
      for (int i = 0; i < MAX_IBUFFS; i++) {
        if (cmd->ibuf_ctx->buff_obj_arr[i].data != NULLBO)
...
static int32_t xma_decoder_init(XmaDecoderSession *dec_session)
{
  ...
  const XmaDecoderProperties *props = (XmaDecoderProperties *)(&(dec_session->decoder_props));
  ...
  if (!(props->width % 2 || props->height % 2))
  {
      ...
      {
        if(!(props->width >= H264_DEC_MIN_WIDTH  && props->height >= H264_DEC_MIN_HEIGHT)   ||
           !(props->width >= H264_DEC_MIN_HEIGHT && props->height >= H264_DEC_MIN_WIDTH)    ||
           !(props->width * props->height >= H264_DEC_MIN_WIDTH * H264_DEC_MIN_HEIGHT))
        {
            xma_logmsg(XMA_ERROR_LOG, XMA_VCU_DECODER, "error :  minimum resolution not met for H264 Decoder  : %d x %d \n", props->width, props->height);
            return XMA_ERROR;
        }
        else if(!(props->width <= H264_DEC_MAX_WIDTH  && props->height <= H264_DEC_MAX_HEIGHT)   ||
                !(props->width <= H264_DEC_MAX_HEIGHT && props->height <= H264_DEC_MAX_WIDTH)   ||
                !(props->width * props->height <= H264_DEC_MAX_WIDTH * H264_DEC_MAX_HEIGHT))
        {
            xma_logmsg(XMA_ERROR_LOG, XMA_VCU_DECODER, "error :  maximum resolution exceeded for H264 Decoder  : %d x %d \n", props->width, props->height);
            return XMA_ERROR;
        }
      }
      else if (props->hwdecoder_type == XMA_HEVC_DECODER_TYPE)
      {
        if(!(props->width >= HEVC_DEC_MIN_WIDTH  && props->height >= HEVC_DEC_MIN_HEIGHT)   ||
           !(props->width >= HEVC_DEC_MIN_HEIGHT && props->height >= HEVC_DEC_MIN_WIDTH))
        {
            ...
            {
              xma_logmsg(XMA_ERROR_LOG, XMA_VCU_DECODER, "error :  minimum resolution not met for HEVC Decoder  : %d x %d \n", props->width, props->height);
              return XMA_ERROR;
            }
        }
        else if(!(props->width <= HEVC_DEC_MAX_WIDTH  && props->height <= HEVC_DEC_MAX_HEIGHT)   ||
                !(props->width <= HEVC_DEC_MAX_HEIGHT && props->height <= HEVC_DEC_MAX_WIDTH))
        {
            ...
            {
              xma_logmsg(XMA_ERROR_LOG, XMA_VCU_DECODER, "error :  maximum resolution exceeded for HEVC Decoder  : %d x %d \n", props->width, props->height);
              return XMA_ERROR;
            }
        }
      }
      ...
      {
        if(!(props->width >= MULTI_DEC_MIN_WIDTH  && props->height >= MULTI_DEC_MIN_HEIGHT)   ||
           !(props->width >= MULTI_DEC_MIN_HEIGHT && props->height >= MULTI_DEC_MIN_WIDTH))
        {
            if(props->width * props->height < MULTI_DEC_MIN_WIDTH * MULTI_DEC_MIN_HEIGHT)
            {
              xma_logmsg(XMA_ERROR_LOG, XMA_VCU_DECODER, "error :  minimum resolution not met for MULTI Decoder  : %d x %d \n", props->width, props->height);
              return XMA_ERROR;
            }
        }
        else if(!(props->width <= MULTI_DEC_MAX_WIDTH  && props->height <= MULTI_DEC_MAX_HEIGHT)   ||
                !(props->width <= MULTI_DEC_MAX_HEIGHT && props->height <= MULTI_DEC_MAX_WIDTH))
        {
            if(props->width * props->height > MULTI_DEC_MAX_WIDTH * MULTI_DEC_MAX_HEIGHT)
            {
              xma_logmsg(XMA_ERROR_LOG, XMA_VCU_DECODER, "error :  maximum resolution exceeded for MULTI Decoder  : %d x %d \n", props->width, props->height);
              return XMA_ERROR;
            }
        }
      }
  ...
  {
    xma_logmsg(XMA_ERROR_LOG, XMA_VCU_DECODER, "error : resolution should be even : %d x %d \n", props->width, props->height);
    return XMA_ERROR;
  }

  ctx->luma_bufsize = (ALIGN(props->width, WIDTH_ALIGN) * ALIGN(props->height, HEIGHT_ALIGN));
  ...
  ret = xma_plg_buffer_read(*(sk_cmd->xma_session), sk_cmd->payload_buf_obj, sizeof(sk_payload_data), 0);
  if (ret != XMA_SUCCESS)
    goto err5;
...
static int32_t xma_decoder_send(XmaDecoderSession *dec_session, XmaDataBuffer *data,
                                int32_t *data_used)
{
  int ret;
  ...
  if (data->alloc_size) {
    memcpy (sk_cmd->ibuf_ctx->buff_obj_arr[sk_cmd->host_to_dev_ibuf_idx].data, data->data.buffer, data->alloc_size);
    ret = xma_plg_buffer_write (xma_session, sk_cmd->ibuf_ctx->buff_obj_arr[sk_cmd->host_to_dev_ibuf_idx], data->alloc_size, 0);
    if (ret != XMA_SUCCESS) {
      xma_logmsg(XMA_ERROR_LOG, XMA_VCU_DECODER, "[%s][%d]xma_dec : failed to do write...\n", __func__, __LINE__);
      return ret;
    }
  }
  ...
  ctx->send_xrt_time += ((rstop.tv_sec - rstart.tv_sec) * 1e6 + (rstop.tv_nsec - rstart.tv_nsec) / 1e3);
#endif
...
static int32_t xma_decoder_recv(XmaDecoderSession *dec_session, XmaFrame *frame)
{
  ...
  if (sk_cmd->obuf_ctx->dev_data->free_index_cnt) {
    uint32_t idx = sk_cmd->obuf_ctx->dev_data->obuff_index[sk_cmd->obuf_ctx->current_free_obuf_index];
    if (idx != 0xBAD) {
      if (ctx->latency_logging) {
        clock_gettime (CLOCK_REALTIME, &ctx->latency);
        ctx->frame_recv++;
      ...
      if (sk_cmd->use_zero_copy)
        frame->data[0].buffer = (XvbmBufferHandle) xvbm_get_buffer_handle(sk_cmd->obuf_ctx->out_pool_handle, idx);
      else {
        ...
        if (!hbuf) {
            xma_logmsg(XMA_ERROR_LOG, XMA_VCU_DECODER, "[%s][%d]xma_dec : invalid host buffer\n", __func__, __LINE__);
            return XMA_ERROR;
        }
        ...
        xvbm_buffer_pool_entry_free (xvbm_get_buffer_handle(sk_cmd->obuf_ctx->out_pool_handle, idx));
      }
      frame->pts = sk_cmd->obuf_ctx->dev_data->obuff_meta[sk_cmd->obuf_ctx->current_free_obuf_index].pts;
  ...
  if (ret != XMA_SUCCESS)
    return ret;
  memcpy( sk_cmd->obuf_ctx->dev_data, sk_cmd->payload_buf_obj.data, sizeof(sk_payload_data));
...
static int32_t xma_decoder_close(XmaDecoderSession *dec_session)
{
  ...
  if (sk_cmd->cu_status == SK_STATUS_BAD) {
    ...
    xma_logmsg(XMA_ERROR_LOG, XMA_VCU_DECODER, "decoder: cu status is bad ! execute 'xbutil reset' to run usecase again\n");
    goto cleanup;
  }
  ...
  cu_cmd = xma_plg_schedule_work_item(*(sk_cmd->xma_session), (char*)(sk_cmd->registers), DEC_REGMAP_SIZE*sizeof(uint32_t), &ret);
...
#else
    uint32_t i;
    for (i = 0; i < sk_cmd->obuf_ctx->buf_count; i++) {
      if (sk_cmd->obuf_ctx->buf_objs[i].data)
        xma_plg_buffer_free (xma_session, sk_cmd->obuf_ctx->buf_objs[i]);
    }

    if (sk_cmd->obuf_ctx->buf_objs)
      free(sk_cmd->obuf_ctx->buf_objs);
#endif
